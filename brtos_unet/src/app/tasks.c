   ...
   for (;;)
   {
       ...
       if (&(server.sender_address) != NULL){
    	   if (memcmp(&(server.sender_address),node_addr64_get(),2) == 0){
    		   ...
    		   // Fazer várias tentativas, pois pode estar respondendo
      ...
      if(ret != TIMEOUT){
       ...
       switch(app_packet.APP_Profile)
       {
        case GENERAL_PROFILE:
          ...
          (void)UARTPutString(UART0_BASE, "Pacote do perfil geral recebido do nï¿½ ");
        ...
        default:
		  #if (UNET_DEVICE_TYPE == PAN_COORDINATOR) && 0
          (void)UARTPutString(UART0_BASE, "Pacote de dados genï¿½rico recebido!\n\r");
		  #endif
          break;
       }
       releaseRadio();
...
void pisca_led(void *param)
{
	...
	// Inicializa o mï¿½dulo de memï¿½ria FLASH
	//InitFlash();

	// Lï¿½ 5 bytes da memï¿½ria FLASH
	//ReadFromFlash(0x1F040, data_test, 5);

	data_test[5]=0;
	status = strcmp(test_pointer, data_test);

	// Verifica se a FLASH jï¿½ havia sido escrita
	if (status)
	{
		// Escreve 5 bytes na memï¿½ria FLASH
	...
	for(;;)
	{
		...
		// Sï¿½ pisca o outro LED se a FLASH leu o valor correto
		if (!status)
		{
		}
		DelayTask(200);
