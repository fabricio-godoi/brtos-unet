...
int mrf24j40_init(const void *isr_handler) {
   ...
   ACTIVITY_LED_DIR_OUT;
	...
	/*
	 * bit 7:3 reserved: Maintain as â€˜0â€™
	 * bit 2   RSTPWR: Power Management Reset bit
	 *         1 = Reset power management circuitry (bit is automatically cleared to â€˜0â€™ by hardware)
	 * bit 1   RSTBB: Baseband Reset bit
	 *         1 = Reset baseband circuitry (bit is automatically cleared to â€˜0â€™ by hardware)
	 * bit 0   RSTMAC: MAC Reset bit
	 *         1 = Reset MAC circuitry (bit is automatically cleared to â€˜0â€™ by hardware)
	 */
	...
	/*
	 * bit 7   FIFOEN: FIFO Enable bit 1 = Enabled (default). Always maintain this bit as a â€˜1â€™.
	 * bit 6   reserved: Maintain as â€˜0â€™
	 * bit 5:2 TXONTS<3:0>: Transmitter Enable On Time Symbol bits(1)
	 *         Transmitter on time before beginning of packet. Units: symbol period (16 Î¼s).
	 *         Minimum value: 0x1. Default value: 0x2 (2 * 16 Î¼s = 32 Î¼s). Recommended value: 0x6 (6 * 16 Î¼s = 96 Î¼s).
	 ...
	 *         in SYMTICKH<7:1>. Units: tick (50 ns). Default value = 0x028 (40 * 50 ns = 2 Î¼s).
	 */
	...
	set_short_add_mem(MRF24J40_BBREG2, 0b01111000); /* Recommended value by the datashet */ //Não tem
	set_short_add_mem(MRF24J40_CCAEDTH, 0b01100000); /* Recommended value by the datashet */ //Não tem
	...
	/* Define os endereços macAddr curto e longo */
	#if 0
	...
	*((uint8_t *) &longaddr + 1) = ROUTER_AUTO_ASSOCIATION_MAC_ADDR;
	...
	// Acho que não precisa do big e little endian aqui
...
void MRF24J40_ISR(void) {
	...
	// Handler do core não limpa flag de interrupções
...
void Radio_Interrupt(void)
{
	...
	// Processa as flags de interrupção
	MRF24J40_ISR();
