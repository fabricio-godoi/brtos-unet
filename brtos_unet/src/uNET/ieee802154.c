...
uint8_t ieee802154_packet_input(packet_t *p)
{
	...
	{
		PRINTF_MAC(1,"FCS error! FCS RX: %d, FCS CALC: %d\r\n",(p->info[PKTINFO_FCSH]<<8) + p->info[PKTINFO_FCSL],
				ieee802154_fcs_calc(&p->packet[MAC_FRAME_CTRL], p->info[PKTINFO_SIZE]));
	...
	/* todo: colocar uma macro p/ desabilitar isto. É usado só para debug. */
...
#endif
	{
		/* todo: futuramente este teste poderá ser feito mais adiante, pois o resultado não é usado aqui. */
