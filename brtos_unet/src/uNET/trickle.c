...
unsigned short random_rand(void){
	// escrever código que use a semente para gerar um número aleatório
	return trickle_seed;
}
#endif

// a semente pode ser o endereço 16 bits do nó + 1 (por causa do coordenador)
void form_seed(uint16_t seed){
	int i = 0;
	while(i < 16 || (seed == 0x0000 || seed == 0x8003)) {
		// o valor que aplica-se o ou na linha abaixo deve ser aleatório
		// não usar o i
		seed = (seed << 1) | i;
		seed <<= 1;
		i++;
...
void run_trickle(trickle_t *timer){
	ostick_t temp;
	// Peguei esse código do contiki, mas me parece que o intervalo cresce
	// rápido demais, talvez a melhor abordagem seja intervalo de escala
	// fixo em 1
	...
	if (timer->interval_scaling < MAX_SCALING_INTERVAL){
		// Ao invés de deixar crescer, limitei em um o fator de escala
		// senão o intervalo cresce muito rápido
	...
	// Calcula o tempo complementar para o próximo cálculo do trickle
