...
static void UNET_radio_isr_handler(void)
{
	...
	{
		radio_txing(FALSE); /* clear RX interrupt flag */
...
void UNET_Radio_Task(void* p)
{
	...
	/* todo: colocar o código abaixo em uma função portável p/ cada plataforma */
#if defined BRTOS_PLATFORM
#if BRTOS_PLATFORM == FRDM_KL25Z
...
void UNET_Router_Down_Ack_Task(void* p)
{
	...
	for(;;)
	{
		...
		{
			/* isto é só para debug e pode ser removido futuramente. */
...
void UNET_Router_Down_Task(void* p)
{
	...
	for(;;)
	{
		...
		/* quantas vezes vale a pena tentar a transmissão? */
...
void UNET_Router_Up_Ack_Task(void* p)
{
	...
	for(;;)
	{
		...
		OSSemPend(Router_Up_Ack_Request,0);
